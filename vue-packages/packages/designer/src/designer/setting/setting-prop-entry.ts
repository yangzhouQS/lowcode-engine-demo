...
import type { GlobalEvent, IPublicApiSetters, IPublicModelEditor, IPublicModelSettingField, IPublicTypeFieldExtraProps, IPublicTypeSetValueOptions } from '@vue3-engine/types';
import { uniqueId, isJSExpression } from '@vue3-engine/utils';
...
export class SettingPropEntry implements ISettingPropEntry {
  ...
  // ==== dynamic properties ====
  private _name = ref<string | number | undefined>(undefined);
  ...
  constructor(readonly parent: ISettingTopEntry | ISettingField, name: string | number | undefined, type?: 'field' | 'group') {
    ...
    this.emitter = {
      on: (event: string, callback: Function) => {
        ...
        if (!this.emitter._events) {
          this.emitter._events = {};
        }
        if (!this.emitter._events[event]) {
          this.emitter._events[event] = [];
        }
        this.emitter._events[event].push(callback);
      },
      emit: (event: string, ...args: any[]) => {
        if (this.emitter._events && this.emitter._events[event]) {
          this.emitter._events[event].forEach((callback: Function) => callback(...args));
        }
      },
      removeListener: (event: string, callback: Function) => {
        if (this.emitter._events && this.emitter._events[event]) {
          this.emitter._events[event] = this.emitter._events[event].filter((cb: Function) => cb !== callback);
        }
      },
      removeAllListeners: () => {
        this.emitter._events = {};
  ...
  setKey(key: string | number) {
    ...
    const propName = this.path.value.join('.');
  ...
  get(propName: string | number) {
    const path = this.path.value.concat(propName).join('.');
  ...
  getPropValue(propName: string | number): any {
    return this.top.getPropValue(this.path.value.concat(propName).join('.'));
  }
...
function isSettingField(obj: any): obj is ISettingField {
