...
import type { IPublicTypeFieldConfig, IPublicTypeSetterConfig } from '@vue3-engine/types';
import { isSetterConfig, isDynamicSetter } from '@vue3-engine/utils';
import type { ISettingField } from './setting-field';

function getHotterFromSetter(setter: any) {
  return setter && (setter.Hotter || (setter.type && setter.type.Hotter)) || [];
}
...
function combineTransducer(transducer: any, arr: any, context: any) {
  ...
  return {
    toHot: (transducer && transducer.toHot || (x: any) => x).bind(context),
    toNative: (transducer && transducer.toNative || (x: any) => x).bind(context),
  };
}
