...
import { cloneDeep, isCustomView, isDynamicSetter, isJSExpression } from '@vue3-engine/utils';
import type { ISettingTopEntry } from './setting-top-entry';
import type { IComponentMeta, INode } from '@vue3-engine/designer';
...
export class SettingField extends SettingPropEntry implements ISettingField {
  ...
  get title() {
    return (
      this._title || (typeof this.name === 'number' ? `Item ${this.name}` : this.name)
    );
  }
  ...
  readonly setter = computed<IPublicTypeSetterType | null>(() => {
    if (!this._setter) {
      return null;
    }
...
/**
 * @deprecated use same function from '@vue3-engine/utils' instead
 */
