...
export class BoxResizing {
  ...
  render() {
    ...
    return (
      <>
        {selecting.map((node) => (
          <BoxResizingForNode key={node.id} node={node} host={this.host} />
        ))}
      </>
    );
  }
...
@observer
export class BoxResizingForNode {
  private host: BuiltinSimulatorHost;
  private node: Node;

  constructor(host: BuiltinSimulatorHost, node: Node) {
    this.host = host;
    this.node = node;
  }
...
@observer
export class BoxResizingInstance {
  private observed: OffsetObserver;
  ...
  constructor(props: { observed: OffsetObserver; highlight?: boolean; dragging?: boolean; designer?: Designer }) {
    this.observed = props.observed;
